    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('.tabelkodebarang').DataTable({
                ...
                ajax:"{{route('listimportaset.index')}}",
                columns: [
            ...
            $('#saveBtn').click(function (e) {
                ...
                $.ajax({
                    data: fd,
                    url: "{{route('listimportaset.store')}}",
                    type: "POST",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
            ...
            $('body').on('click', '.deletekodebarang', function () {
                var idkodebarang = $(this).data("id");
                if(confirm("Apakah Anda Yakin AKan Hapus Data Ini!")){
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('listimportaset.destroy','') }}"+'/'+idkodebarang,
                        success: function (data) {
                            if (data.status == "berhasil"){
                                Swal.fire({
                                    title: 'Sukses',
            ...
            $('body').on('click', '.importtransaksi', function () {
                var kodebarang = $(this).data("id");
                if(confirm("Apakah Anda Yakin AKan Import Data Transaksi Kode Barang Ini!")){
                    window.location="{{URL::to('importtransaksiaset')}}"+"/"+kodebarang;
                }
            });
