                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form id="formsubmenu" name="formsubmenu" class="form-horizontal">
                                            ...
                                            <div class="form-group">
                                                <label for="Uraian SubMenu" class="col-sm-6 control-label">Uraian PPK</label>
                                                <div class="col-sm-12">
                                                    <input type="text" class="form-control" id="uraianppk" name="uraianppk" placeholder="Masukan Uraian PPK" value="" maxlength="100" required="">
                                                </div>
                                            </div>
                                            <div class="form-group">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('.tabelsubmenu').DataTable({
                ...
                ajax:"{{route('ppksatker.index')}}",
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                    {data: 'kodesatker', name: 'kodesatker'},
                    {data: 'uraianppk', name: 'uraianppk'},
            ...
            $('body').on('click', '.edit', function () {
                ...
                $.get("{{ route('ppksatker.index') }}" +'/' + idsubmenu +'/edit', function (data) {
                    $('#modelHeading').html("Edit Sub Menu");
                    $('#saveBtn').val("edit");
                    $('#ajaxModel').modal('show');
                    $('#id').val(data.id);
                    $('#kodesatker').val(data.kodesatker).trigger('change');
                    $('#uraianppk').val(data.uraianppk);
                    if (data.status == "on"){
            ...
            $('#saveBtn').click(function (e) {
                ...
                $.ajax({
                    data: fd,
                    url: saveBtn === "tambah" ? "{{route('ppksatker.store')}}":"{{route('ppksatker.update','')}}"+'/'+id,
                    type: "POST",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
            ...
            $('body').on('click', '.delete', function () {
                ...
                if(confirm("Apakah Anda Yakin AKan Hapus Data Ini!")){
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('ppksatker.destroy','') }}"+'/'+idsubmenu,
                        success: function (data) {
                            if (data.status == "berhasil"){
                                Swal.fire({
                                    title: 'Sukses',
