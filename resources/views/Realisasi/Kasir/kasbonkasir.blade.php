                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form id="formkasbon" name="formkasbon" class="form-horizontal">
                                            ...
                                            <div class="form-group">
                                                <label for="nilaipencairankasir" class="col-sm-6 control-label">Nilai Pencairan</label>
                                                <div class="col-sm-12">
                                                    <input type="number" class="form-control nilaipencairankasir" id="nilaipencairankasir" name="nilaipencairankasir" placeholder="Nilai Pencairan" value="" maxlength="21">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label for="keterangankasir" class="col-sm-6 control-label">Keterangan Kasir</label>
                                                <div class="col-sm-12">
                                                    <input type="text" class="form-control keterangankasir" id="keterangankasir" name="keterangankasir" placeholder="Keterangan Kasir" value="" maxlength="500" required>
                                                </div>
                                            </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('body').on('click', '.prosestransaksi', function () {
                var id = $(this).data('id');
                $.get("{{ route('editkasbonkasir','') }}" +'/' + id , function (data) {
            ...
            $('body').on('click', '.prosespertanggungjawaban', function () {
                var id = $(this).data('id');
                $.get("{{ route('prosespertanggungjawabankasir','') }}" +'/' + id , function (data) {
                    $('#modelHeading').html("Proses Pertanggungjawaban");
                    $('#saveBtn').val("edit");
                    $('#ajaxModel').modal('show');
                    $('#id').val(data.id);
                    $('#kdsatker').val(data.kdsatker).trigger('change');
                    ...
                    $('#peruntukan').val(data.peruntukan);
                    $('#nilaipengajuan').val(data.nilaipengajuan);
                    $('#nilaipencairankasir').val(data.nilaipencairankasir).readOnly;
                    $('#pagu').val(data.pagu);
                    $('#realisasisaatini').val(data.realisasisaatpengajuan);
                    $('#sisapagu').val(data.sisapagu);
                })
            ...
            $('#saveBtn').click(function (e) {
                ...
                $.ajax({
                    data: fd,
                    url: "{{route('prosespengajuankasbonkasir')}}",
                    type: "POST",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
