        ...
        if ($request->ajax()) {
            $data = DB::table('indikatorro as a')
                ->select([DB::raw('concat(a.tahunanggaran,".",a.kodesatker,".",a.kodekegiatan,".",
                    ...
                    a.uraianindikatorro) as indikatorro'),'f.uraianbagian as uraianbagian', 'a.target as target','a.idkro as idkro','a.idro as idro','e.uraianro as uraianro',
        ...
        foreach ($dataindikatorro as $dri){
            ...
            if ($adarealisasi <1){
                ...
                if ($idbulan == 1){
                    ...
                    if ($adadata > 0){
                        $dataperiodesebelumnya = $dataperiodesebelumnya->get();
                        //json_encode($dataperiodesebelumnya); //$dataperiodesebelumnya;
                        ...
                        $jumlahsdperiodesebelumnya = DB::table('realisasiindikatorro')
                            ->select([DB::raw('sum(jumlah) as jumlahsdperiodeini')])
                            ->where('periode','<=',DB::raw($idbulan-1))
                            ->where('tahunanggaran','=',$tahunanggaran)
                            ->where('idindikatorro','=',$idindikatorro)
