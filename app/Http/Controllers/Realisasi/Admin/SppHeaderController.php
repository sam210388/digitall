...
class SppHeaderController extends Controller
{
    ...
    private function cekstatusspp(){
        $tahunanggaran = session('tahunanggaran');
        $dataspp = DB::table('sppheader')->where('THN_ANG','=',$tahunanggaran)->get();
        foreach ($dataspp as $data){
            $ID_SPP = $data->ID_SPP;
            $STATUSPENGELUARAN = $data->STATUS_PENGELUARAN;
            if($STATUSPENGELUARAN == 1){
                $this->tarikdatapengeluaran($ID_SPP);
                $this->tarikdatapotongan($ID_SPP);
            }
