...
class GedungController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('dokumendbr',function ($row){
                    if (Storage::disk('public')->missing('/pengesahandbrfinal/PengesahanDBRGedung'.$row->id.'.pdf')){
                        $linkpengesahan = "File Tidak Ada";
                    }else{
                        $linkpengesahan = '<a href="'.env('APP_URL')."/".asset('storage')."/pengesahandbrfinal/PengesahanDBRGedung".$row->id.'.pdf" >Download Pengesahan</a>';
                    }

                    //$datalokasidbrfinal = getenv('APP_URL')."/".asset('storage')."/dbrfinaldigitall/DBRRuangan".$row->iddbr.".pdf";
                    if (Storage::disk('public')->missing('/dbrfinaldigitall/DBRGedung'.$row->id.'.pdf')){
                        $linkdokumendbr = "File Tidak Ada";
                    }else{
                        $linkdokumendbr = '<a href="'.env('APP_URL')."/".asset('storage')."/dbrfinaldigitall/DBRGedung".$row->id.'.pdf" >Download DBR</a>';
                    }
    ...
    public function cetakdbrgedung($idgedung){
        ...
        $idpenanggungjawab = DB::table('gedung')->where('id','=',$idgedung)->value('idpenanggungjawab');
        $datapenanggungjawab = DB::table('pegawai')->where('id','=',$idpenanggungjawab)->get();
        foreach ($datapenanggungjawab as $dp){
            $penanggungjawab = $dp->nama;
            $nip = $dp->nip;
        }

        $waktusetujudbr = DB::table('dbrinduk')->where('idgedung','=',$idgedung)->limit(1)->value('tanggalpersetujuandbr');
        $dataqrunit = "Disetujui Oleh: ".$penanggungjawab." Pada: ".$waktusetujudbr;
        QrCode::generate($dataqrunit,asset('storage/qrunit/DBRGedung'.$idgedung.'.svg'));
        ...
        $this->cetakpengesahandbrgedung($idgedung);
        return $pdf->stream('DBRGedung'.$idgedung.'.pdf');
    }

    public function cetakpengesahandbrgedung($idgedung){
        $datareferensidbr = DB::table('gedung as a')
            ->select(['a.uraiangedung as gedung',
                'b.uraianarea as area','c.uraiansubarea as subarea'])
            ->leftJoin('area as b','b.id','=','a.idarea')
        ...
        $listbarang = $datadetildbr->get();
        $jumlahbarang = $datadetildbr->count();
        $pdf = Pdf::loadView('laporan.sirangga.pengesahandbrgedung',[
            'datareferensidbr' => $datareferensidbr,
        ...
        Storage::put('public/pengesahandbrfinal/PengesahanDBRGedung'.$idgedung.'.pdf', $pdf->output());
        //return $pdf->stream('DBRGedung'.$idgedung.'.pdf');
    }
