...
class DetilDBRController extends Controller
{
    ...
    function exportdetildbr($statusbarang){
        return Excel::download(new ExportDetilDBR($statusbarang),'DetilDBR.xlsx');
    }

    public function getDataDetilBDR()
    {
        $model = DetilDBRModel::with('dbrindukrelation')
            ->select('detildbr.*');
