...
class DetilKontraktualImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            $nilai_distribusi_akselerasi = $row[10];
            $nilai_kontrak_dini = $row[11];
            $nilai_akselerasi_53 = $row[12];

            //$akum_nilai_ketepatan_waktu = $row[15];
            $akum_nilai_distribusi = $row[13];
            $akum_nilai_kontrak_dini = $row[14];
            $akum_nilai_akselerasi_53 = $row[15];

            //dapatkan id bagian dari kontrak coa, dengan terlebih dahulu melookup kontrak coa
            $data_kontrak = DB::table('kontrakheader as a')->where('NO_KONTRAK','=',$no_kontrak);
            $jumlahkontrak = $data_kontrak->count();
            if ($jumlahkontrak > 1 ){
                //dapatkan $id kontrak yg ada idbiro nya
                $rinciankontrak = $data_kontrak->get();
                $idbagian = 0;
                $idbiro = 0;
                foreach ($rinciankontrak as $r){
                    $ID_KONTRAK = $r->ID_KONTRAK;
                    $idbagian = DB::table('kontrakcoa')->where('ID_KONTRAK','=',$ID_KONTRAK)->value('idbagian');
                    $idbiro = DB::table('kontrakcoa')->where('ID_KONTRAK','=',$ID_KONTRAK)->value('idbiro');
                    if ($idbagian || $idbiro){
                        break;
                    }
                }
            }else{
                $ID_KONTRAK = DB::table('kontrakheader as a')->where('NO_KONTRAK','=',$no_kontrak)->value('ID_KONTRAK');
                $idbagian = DB::table('kontrakcoa')->where('ID_KONTRAK','=',$ID_KONTRAK)->value('idbagian');
                $idbiro = DB::table('kontrakcoa')->where('ID_KONTRAK','=',$ID_KONTRAK)->value('idbiro');
            }
           ...
           $datainsert = array(
               ...
               'nilai_distribusi_akselerasi' => $nilai_distribusi_akselerasi,
               'nilai_kontrak_dini' => $nilai_kontrak_dini,
               'nilai_akselerasi_53' => $nilai_akselerasi_53,
               'akum_nilai_distribusi' => $akum_nilai_distribusi,
